...
- Kanama release line: `0.5.0` (tagged 2026-09-16; a pre-1.0 preview baseline).
...
- The root is ONE `kotlin("multiplatform")` module (task 104 step 3) with targets
  `jvm()`, `iosArm64()` and `iosSimulatorArm64()`:
    - `src/commonMain/kotlin`: the KMP common fragment — the 19 value types
      (`types/`), `api/GodotHandle.kt`, and the `expect` seams
      (`binding/runtime/RawSegment.expect.kt`, `BuiltinCalls.expect.kt`, the
      GENERATED `ObjectCalls.expect.kt`) plus the generated `types/Real.kt`. It
      compiles on its own (`compileCommonMainKotlinMetadata`), which is what makes
      the compiler the proof that both backends implement every seam.
    ...
    - `src/iosMain/kotlin`: the Kotlin/Native iOS runtime (see below), `src/jvmTest`:
      the JVM tests, `src/nativeInterop`: the `kanama_ios` cinterop def.
...
- `ios`: the iOS C shim and headers. The Kotlin/Native runtime lives in
  ...
  collision classes, `<Class>.ios.kt` companions); the shared tree comes from
  `src/sharedApi` and the seams from `src/commonMain`.
...
|---|---|---|
| `src/jvmMain/kotlin/binding/runtime/ObjectCalls.kt` | hand-written (desktop/Android) | The ptrcall helper surface the generated wrappers call. Not generated: `scripts/audit_generator_shape_policy.py` gates its shapes against the generator's call-shape table, and its iOS counterpart `src/iosMain/.../binding/runtime/ObjectCalls.kt` is hand-written outside the `GENERATED MEMBERS` region the generator owns at the end of the object (task 104 step 3b). Both are `actual object ObjectCalls` since step 3cd: the compiler holds them to the GENERATED `expect object` in `src/commonMain/.../ObjectCalls.expect.kt`, and `scripts/check_objectcalls_parity.py` holds that file to the set of helpers the tree calls. |
| The generated wrapper tree: `src/sharedApi/kotlin/.../api/*.kt` (shared), `src/jvmMain/kotlin/.../api/<Class>.jvm.kt` companions and the desktop-only classes, `src/iosMain/.../api/<Class>.ios.kt` companions and the iOS-only classes, `src/commonMain/.../binding/runtime/ObjectCalls.expect.kt`, `docs/reference/generated/ios-shape-gap.md` | `scripts/generate_api_wrapper.py --write-tree` (then `sync_kdoc_from_godot_docs.py --write`) | Public wrappers must be regeneratable unless listed as hand-shaped in `PER_PLATFORM_WRAPPERS`; `check_wrapper_generator.py` fails on any hand edit. |
