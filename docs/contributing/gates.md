...
Generated by `python3 scripts/generate_gates_index.py --markdown docs/contributing/gates.md`; `--check` runs as a `local_ci.sh` stage, so this page cannot drift from the scripts and workflows it lists.
...
| 7 | `gates index docs check` | Generate docs/contributing/gates.md: every gate, what it proves, where it runs (task 99, R18). | PR + push to main (ci.yml `local-ci`); local | `scripts/generate_gates_index.py` | 2026-09-09 |
