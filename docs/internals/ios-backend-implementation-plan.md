...
| Task | Owner | Status | Done by | Validate | Guardrail |
|---|---|---|---|---|---|
| T5.1 Measure platformer frame time + per-frame script/binding time (instrument `_physics_process`, e.g. monotonic timestamps around the script call path) on iPhone 12 **and** 15 Pro | S | ◐ | Opus 2026-06-10 (iPhone 12 measured) | **iPhone 12 (A14, the floor): Kanama script+binding overhead = ~0.63 ms/frame** (≈28 script-calls/frame, ~22 µs/call incl. the user Kotlin body — pure binding crossing is sub-µs). Instrumented in `KanamaIosRuntime.frame()`/dispatch with `TimeSource.Monotonic`, aggregate-only log (per-call logging silenced — the instrument was reverted after measuring, commit-free). User-confirmed **smooth** on the 12. **15 Pro not yet measured** (was unplugged; will be ≤ the 12). fps-total readout via `Engine.get_frames_per_second` was anomalous in the short console window (settling post-load) — re-capture with init logs silenced. | per-call logging OFF ✅ |
| T5.2 Compare vs targets; if over budget apply the architecture-doc perf guidance (cache `MethodBind`, avoid per-call `StringName` alloc, prefer ptrcall over Variant `call`, fewer boundary crossings) and re-measure | O | ✅ | Opus 2026-06-10 | **PASS — well under budget.** 0.63 ms/frame ≈ **3.8% of the 16.6 ms (60fps) budget** on the floor device; smooth play confirmed. No optimization needed now (the per-call `memScoped` buffer-alloc — the FFM gap — never bit; pooling is a future lever if a scene does thousands of ObjectCalls/frame). **Perf-viability conclusion: the Kanama binding is NOT a bottleneck (FFM-class: AOT + cached typed ptrcall, comparable to C#, can beat interpreted GDScript).** | no Match3 regression |
