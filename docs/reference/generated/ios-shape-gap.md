...
**Gap:** 2 of 1003 shared classes carry a desktop companion with members waiting on a helper; 3 desktop-only members waiting; 1 distinct `ObjectCalls` helpers and 1 wrapper types waited on; 0 properties read-only in the shared tree because only their setter is desktop-only; 3 desktop-only members by design.
