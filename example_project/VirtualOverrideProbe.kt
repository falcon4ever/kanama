...
/**
 * Phase 5.2 probe: exercises `@OverrideVirtual` for arbitrary, value-returning engine virtuals
 * beyond the fixed lifecycle set. Attaches to `Control` and overrides several of its virtuals — a
 * `Vector2` return, a `Vector2`-in/`Bool` return, and (task 13) a `Vector2`-in/`String` return — so
 * the generated registrar must validate each signature against the engine table and emit the
...
class VirtualOverrideProbe(godotObject: MemorySegment) :
  ...
  @OverrideVirtual fun _get_tooltip(at: Vector2): String = "tip@${at.x.toInt()},${at.y.toInt()}"
  ...
  @OverrideVirtual fun _get_focused_accessibility_element(): RID = RID(0L)
}
