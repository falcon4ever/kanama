...
internal interface KanamaIosScriptBridge {
  ...
  fun callV(methodName: String, args: List<Any?>): Boolean = false
  ...
  fun callVReturning(methodName: String, args: List<Any?>): Any? = KanamaIosNoReturn

  fun setProperty(propertyIndex: Int, value: Long): Boolean = false

  fun getProperty(propertyIndex: Int): Any? = KanamaIosNoProperty

  fun setPropertyString(propertyIndex: Int, value: String): Boolean = false
...
internal object KanamaIosProjectRegistry {
  ...
  fun descriptor(path: String): KanamaIosScriptDescriptor? = descriptors[path]
}
...
internal object KanamaIosRuntime {
  ...
  fun frame() {
    ...
    frameCount += 1
    if (
      KanamaIosGodot.setFirstNodeInGroupText(PROBE_GROUP, "Kanama iOS Kotlin/Native frame ready")
    ) {
  ...
  fun createScriptResource(path: String?): Long {
    ...
    scriptResources[handle] = IosScriptResource(path = normalizedPath, descriptor = descriptor)
    if (descriptor == null) {
  ...
  fun scriptResourceRpcConfigCallLocal(handle: Long, rpcConfigIndex: Int): Int =
    if (
      scriptResources[handle]?.descriptor?.rpcConfigs?.getOrNull(rpcConfigIndex)?.callLocal == true
    )
      1
    else 0
  ...
  fun callScriptInstanceReturning(handle: Long, methodIndex: Int, args: List<Any?>): Any? {
    ...
    val method =
      instance.resource.descriptor?.methods?.getOrNull(methodIndex) ?: return KanamaIosNoReturn
  ...
  fun callScriptInstanceV(handle: Long, methodName: String, args: List<Any?>): Boolean {
    ...
    if (ok && shouldLogScriptMethodCall(methodName)) {
      val kind =
        if (instance.resource.path == PROBE_SCRIPT_PATH) "built-in probe" else "project script"
  ...
  fun setScriptInstancePropertyArray(handle: Long, propertyIndex: Int, values: LongArray): Boolean {
    ...
    if (ok) {
      log(
        "property array set handle=$handle index=$propertyIndex count=${values.size} path=${instance.resource.path}"
      )
    }
    return ok
  }

  fun setScriptInstancePropertyIntArray(
    handle: Long,
    propertyIndex: Int,
    values: LongArray,
  ): Boolean {
    ...
    if (ok) {
      log(
        "property int-array set handle=$handle index=$propertyIndex count=${values.size} path=${instance.resource.path}"
      )
    }
    return ok
  }

  fun setScriptInstancePropertyStringArray(
    handle: Long,
    propertyIndex: Int,
    values: List<String>,
  ): Boolean {
    ...
    if (ok) {
      log(
        "property string-array set handle=$handle index=$propertyIndex count=${values.size} path=${instance.resource.path}"
      )
    }
    return ok
  }
...
fun kanamaIosRuntimeScriptResourceBaseType(
  scriptHandle: Long,
...
fun kanamaIosRuntimeScriptResourceMethodName(
  ...
  writeCString(
    KanamaIosRuntime.scriptResourceMethodName(scriptHandle, methodIndex),
    buffer,
    bufferSize,
  )
}
...
fun kanamaIosRuntimeScriptResourcePropertyName(
  ...
  writeCString(
    KanamaIosRuntime.scriptResourcePropertyName(scriptHandle, propertyIndex),
    buffer,
    bufferSize,
  )
}
...
fun kanamaIosRuntimeScriptResourcePropertyHintString(
  ...
  writeCString(
    KanamaIosRuntime.scriptResourcePropertyHintString(scriptHandle, propertyIndex),
    buffer,
    bufferSize,
  )
}
...
fun kanamaIosRuntimeScriptResourceSignalName(
  ...
  writeCString(
    KanamaIosRuntime.scriptResourceSignalName(scriptHandle, signalIndex),
    buffer,
    bufferSize,
  )
}
...
fun kanamaIosRuntimeScriptResourceRpcConfigMethodName(
  ...
  writeCString(
    KanamaIosRuntime.scriptResourceRpcConfigMethodName(scriptHandle, rpcConfigIndex),
    buffer,
    bufferSize,
  )
}
...
fun kanamaIosRuntimeScriptResourceRpcConfigTransferMode(
  scriptHandle: Long,
  rpcConfigIndex: Int,
): Int = KanamaIosRuntime.scriptResourceRpcConfigTransferMode(scriptHandle, rpcConfigIndex)
...
// generator.
@Suppress("unused")
private const val IOS_PT_TYPED_ARRAY_BLOB = 39 // 38 is IOS_PT_VARIANT (parcel 7)
...
internal fun decodeIosCallArg(tag: Int, ptr: CPointer<ByteVar>?): Any? {
  ...
  return when (tag) {
    IOS_PT_INT64,
    IOS_PT_OBJECT -> ptr.reinterpret<LongVar>()[0]
...
private fun encodeIosReturn(value: Any?, retTag: CPointer<IntVar>?, retBuf: CPointer<ByteVar>?) {
  ...
  when (value) {
    ...
    is Double -> {
      retBuf.reinterpret<DoubleVar>()[0] = value
    ...
    is DoubleArray -> {
      retBuf.reinterpret<CPointerVar<ByteVar>>()[0] =
        IosReturnPackedDescScratch.encodeDoubles(value)
    ...
    is Map<*, *> -> {
      retBuf.reinterpret<CPointerVar<ByteVar>>()[0] =
        IosReturnContainerScratch.encodeDictionary(value)
    ...
    is List<*> ->
      when (value.firstOrNull()) {
        ...
        is Color -> {
          ...
          retBuf.reinterpret<CPointerVar<ByteVar>>()[0] =
            IosReturnContainerScratch.encodeArray(value)
...
private object IosReturnStringScratch {
  ...
  fun encode(value: String): CPointer<ByteVar> {
    ...
    val existing = buf
    val b: CPointer<ByteVar> =
      if (existing == null || capacity < needed) {
...
internal object IosReturnContainerScratch {
  ...
  private fun int64Bytes(v: Long): ByteArray =
    ByteArray(8) { i -> ((v ushr (i * 8)) and 0xFF).toByte() }
  ...
  fun selfTestParse(
    blob: CPointer<ByteVar>,
    isDictionary: Boolean,
  ): List<Triple<String, Int, ByteArray>> {
    var off = 0
    fun int32LE(): Int {
      val v =
...
/**
 * OBJECTCALLS-SELFTEST hooks for the task-29 return encoders. Each encodes through the same scratch
 * the live return path uses and parses the buffer back the way the C side reads it, so the
 * on-device rows assert element widths and boundaries without needing an engine call.
 */
@OptIn(ExperimentalForeignApi::class)
internal fun kanamaIosVirtualPackedIntsReturnSelfTest(values: IntArray): List<Int> {
  val (count, bytes) =
    IosReturnPackedDescScratch.selfTestReadBack(IosReturnPackedDescScratch.encodeInts(values), 4)
...
internal fun kanamaIosVirtualPackedLongsReturnSelfTest(values: LongArray): List<Long> {
  val (count, bytes) =
    IosReturnPackedDescScratch.selfTestReadBack(IosReturnPackedDescScratch.encodeLongs(values), 8)
  if (count.toInt() != values.size) return emptyList()
  return List(values.size) { i ->
    var v = 0L
...
internal fun kanamaIosVirtualPackedBytesReturnSelfTest(values: ByteArray): ByteArray {
  val (count, bytes) =
    IosReturnPackedDescScratch.selfTestReadBack(IosReturnPackedDescScratch.encodeBytes(values), 1)
...
internal fun kanamaIosVirtualPackedDoublesReturnSelfTest(values: DoubleArray): List<Double> {
  val (count, bytes) =
    IosReturnPackedDescScratch.selfTestReadBack(IosReturnPackedDescScratch.encodeDoubles(values), 8)
  if (count.toInt() != values.size) return emptyList()
  return List(values.size) { i ->
    var v = 0L
    for (j in 0 until 8) v = v or ((bytes[i * 8 + j].toLong() and 0xFF) shl (j * 8))
...
internal fun kanamaIosVirtualPackedVector3ReturnSelfTest(values: List<Vector3>): List<Vector3> {
  val (count, bytes) =
    IosReturnPackedDescScratch.selfTestReadBack(
      IosReturnPackedDescScratch.encodeVector3s(values),
      12,
    )
  if (count.toInt() != values.size) return emptyList()
...
// OBJECTCALLS-SELFTEST hook for Variant virtual returns (task 13). A Variant-returning virtual
// hands
