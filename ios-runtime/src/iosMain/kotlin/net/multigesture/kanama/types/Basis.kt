...
/**
 * A 3×3 matrix for 3D rotation and scale, as three column axes (x, y, z). Kanama value types are
 * immutable snapshots; assign a new value back to the Godot property after changing components.
 *
 * iOS marshalling note: at ptrcall a Basis is 9 `real_t` (float32 on single-precision iOS) in
 * column-major order — `[x.x, y.x, z.x, x.y, y.y, z.y, x.z, y.z, z.z]` — laid out by the generated
 * ObjectCalls helpers. Engine-computed value methods (e.g. [inverse]) route through the
 * BuiltinCalls value-type call path.
 */
data class Basis(val x: Vector3, val y: Vector3, val z: Vector3) {
  /**
  ...
  /**
   * Returns a copy of this basis scaled by [scale] (each row multiplied by the matching [scale]
   * component). IDENTITY scaled by (sx,sy,sz) is diag(sx,sy,sz).
   */
  ...
  /**
   * Returns the determinant of this basis, computed by Godot (scalar `real_t` return decoded to
   * Double). A negative determinant means the basis flips orientation.
   */
  ...
  companion object {
    ...
    /**
     * Returns a rotation basis that points the -Z axis toward [target] with +Y toward [up] (or the
     * model-front axis when [useModelFront]). `Basis.looking_at` is a *static* builtin, so the call
     * passes an empty base (mirrors the desktop `base = NULL`).
     */
    fun lookingAt(
      target: Vector3,
