...
internal data class IosScriptInput(val model: ScriptModel, val resourcePath: String) {
  val packageName: String?
    get() = model.fqName.substringBeforeLast('.', "").ifEmpty { null }

  val className: String
    get() = model.simpleName
}

// ---------- thin per-script models (mirror the old build.gradle.kts data classes) ----------

private val lifecycleIosVirtuals =
  setOf(
    "_ready",
    "_enter_tree",
    "_exit_tree",
...
internal class IosScriptCodeEmitter(
  ...
  fun registrySource(): String {
    ...
    scripts.forEach { script ->
      ...
      if (readableProperties.isNotEmpty()) {
        builder.appendLine()
        builder.appendLine(
          "    override fun getProperty(propertyIndex: Int): Any? = when (propertyIndex) {"
        )
      ...
      if (valueMethods.isNotEmpty()) {
        builder.appendLine(
          "    override fun callVReturning(methodName: String, args: List<Any?>): Any? = when (methodName) {"
        )
      ...
      builder.appendLine(
        "    override fun setProperty(propertyIndex: Int, value: Long): Boolean = when (propertyIndex) {"
      )
      script.properties.forEachIndexed { index, property ->
        if (property.isObjectType && property.godotClassName.isNotEmpty()) {
          ...
          builder.appendLine(
            "        $index -> { script.${property.kotlinName} = if (value != 0L) net.multigesture.kanama.ios.iosScriptInstanceForOwner(value) as? ${property.customScriptFqName} else null; true }"
          )
        } else if (property.scalarSetExpression.isNotEmpty()) {
          builder.appendLine(
            "        $index -> { script.${property.kotlinName} = ${property.scalarSetExpression}; true }"
          )
        }
      }
      builder.appendLine("        else -> false")
      ...
      val stringProperties =
        script.properties.filter { !it.isObjectType && !it.isList && it.godotClassName == "String" }
      if (stringProperties.isNotEmpty()) {
        builder.appendLine()
        builder.appendLine(
          "    override fun setPropertyString(propertyIndex: Int, value: String): Boolean = when (propertyIndex) {"
        )
      ...
      val listProperties =
        script.properties.filter {
          it.isList &&
            (it.listElementClassName.isNotEmpty() || it.arrayElementCustomScriptFqName.isNotEmpty())
        }
      if (listProperties.isNotEmpty()) {
        builder.appendLine()
        builder.appendLine(
          "    override fun setPropertyObjectArray(propertyIndex: Int, values: LongArray): Boolean = when (propertyIndex) {"
        )
        script.properties.forEachIndexed { index, property ->
          when {
            ...
            property.isList && property.arrayElementCustomScriptFqName.isNotEmpty() -> {
              builder.appendLine(
                "        $index -> { script.${property.kotlinName} = values.toList().mapNotNull { owner -> net.multigesture.kanama.ios.iosScriptInstanceForOwner(owner) as? ${property.arrayElementCustomScriptFqName} }${property.mutableSuffix()}; true }"
              )
            }
      ...
      if (enumListProperties.isNotEmpty()) {
        builder.appendLine()
        builder.appendLine(
          "    override fun setPropertyIntArray(propertyIndex: Int, values: LongArray): Boolean = when (propertyIndex) {"
        )
        script.properties.forEachIndexed { index, property ->
      ...
      if (stringListProperties.isNotEmpty()) {
        builder.appendLine()
        builder.appendLine(
          "    override fun setPropertyStringArray(propertyIndex: Int, values: List<String>): Boolean = when (propertyIndex) {"
        )
        script.properties.forEachIndexed { index, property ->
          if (property.isList && property.listElementIsString) {
            builder.appendLine(
              "        $index -> { script.${property.kotlinName} = values${property.mutableSuffix()}; true }"
            )
          }
        }
        builder.appendLine("        else -> false")
      ...
      if (valueProperties.isNotEmpty()) {
        builder.appendLine()
        builder.appendLine(
          "    override fun setPropertyValue(propertyIndex: Int, value: Any): Boolean = when (propertyIndex) {"
        )
        script.properties.forEachIndexed { index, property ->
          if (property.valueTypeClassName.isNotEmpty()) {
            builder.appendLine(
              "        $index -> { script.${property.kotlinName} = value as ${property.valueTypeClassName}; true }"
            )
          }
        }
        builder.appendLine("        else -> false")
  ...
  fun constantsSource(): String {
    ...
    builder.appendLine(
      "private fun emitIosSignal(instance: Any, signalName: String, args: Array<out Any?>) {"
    )
    builder.appendLine(
      "    val script = instance as? net.multigesture.kanama.api.KanamaScript<*> ?: return"
    )
    builder.appendLine(
    ...
    scripts.forEach { script ->
      ...
      if (helperMethods.isNotEmpty()) {
        ...
        helperMethods.forEach { method ->
          ...
          val typedParams =
            method.args.mapIndexed { i, a -> ", a$i: ${a.kotlinType}" }.joinToString("")
          ...
          builder.appendLine()
          builder.appendLine(
            "    fun $functionName(target: net.multigesture.kanama.api.GodotObject$typedParams): Boolean {"
          )
          builder.appendLine(
            "        val instance = target.kotlinScriptInstance<$fqClassName>() ?: return false"
          )
      ...
      if (script.rpcConfigs.isNotEmpty()) {
        ...
        script.rpcConfigs.forEach { rpc ->
          ...
          val suffix =
            constantIdentifier(rpc.godotName).removeSurrounding("`").replaceFirstChar {
              it.uppercase()
            }
          ...
          if (rpc.callLocal) {
            ...
            builder.appendLine(
              "        net.multigesture.kanama.api.Node(instance.godotObject).callLocalRpc($godotLit$forwardArgs)"
            )
            builder.appendLine("    }")
          }
          builder.appendLine()
  ...
  private fun IosScriptInput.toIosScript(): IosScript {
    ...
    val signals =
      model.signals.map { IosSignal(it.godotName, it.godotName) }.distinctBy { it.godotName }
    val rpcConfigs =
      model.methods
        .mapNotNull { method ->
          method.rpc?.let { rpc ->
  ...
  private fun ScriptPropertyModel.toIosProperty(className: String): IosProperty {
    ...
    val customScript =
      if (type == TypeMapping.OBJECT && customScriptFqName != null) customScriptFqName else ""
  ...
  private fun godotVariantTypeFor(type: TypeMapping): Int =
    when (type) {
      TypeMapping.BOOL -> 1
      TypeMapping.INT -> 2
      TypeMapping.FLOAT -> 3
