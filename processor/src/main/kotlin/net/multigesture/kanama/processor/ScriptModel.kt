...
internal data class SignalModel(val godotName: String, val args: List<ArgModel>)
...
internal data class VirtualModel(
  ...
  /**
   * Return type for value-returning virtuals (e.g. `_get_minimum_size` -> Vector2). `null` for
   * `void` virtuals (all the lifecycle ones). Set for `@OverrideVirtual` declarations; drives
...
/**
 * Narrow Kotlin scalar surfaced for a 64-bit Godot Variant slot. Godot floats and ints are always
 * 64-bit on the wire; a `kotlin.Float`/`kotlin.Int` property maps to Variant FLOAT/INT with
 * widening on get and narrowing on set. Both the JVM and iOS emitters generate the corresponding
 * conversion.
 */
...
internal data class ScriptPropertyModel(
  ...
  /**
   * Set for Kotlin `enum class` properties (task 32, issue #37). The Variant slot is INT carrying
   * the ordinal (C# export parity); registration adds PROPERTY_HINT_ENUM with [enumEntries] as the
   * hint string so the inspector renders a dropdown.
   */
  ...
  /**
   * Set for `List<enum class>` properties (task 38, issue #40). The Variant slot is a typed int
   * Array (hint string `"2/2:<entries>"`) carrying ordinals per element — the array form of the
   * [enumFqName] scalar export (C# enum-array parity).
   */
   ...
   * [mapKey*]/[mapValue*] fields are the dictionary analogue of the [arrayElement*] fields and are
   * non-null only when the property is a typed Map. A plain `Map<String, Any?>` export stays
   ...
   * engine-backed `Godot.Collections.Dictionary` preserves a key whose value is nil. When true, a
   * nil / wrong-typed value decodes to `null` and the key is preserved; when false, Kotlin's type
   * system cannot hold a null there, so the entry is dropped (like the typed-array nil-drop).
   */
