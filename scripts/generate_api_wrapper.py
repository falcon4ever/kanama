...
# guessed — see docs/contributing/backends/ios.md §"Rules".
#
...
IOS_OBJECTCALLS = (
    ROOT / "src/iosMain/kotlin/net/multigesture/kanama/binding/runtime/ObjectCalls.kt"
)
DESKTOP_OBJECTCALLS = ROOT / "src/jvmMain/kotlin/binding/runtime/ObjectCalls.kt"
...
# The iOS companion files (`<Class>.ios.kt`) star-import the package: their iOS-only sugar can call
# a hand-written helper that is still an `ObjectCalls` EXTENSION, and an extension needs the import.
# The SHARED tree does not -- task 104 step 3 parcel C' dropped the line from it after measuring
# that every symbol those files use (RawSegment, NULL_SEGMENT, ObjectCalls, BuiltinCalls) is
# imported by name.
...
KANAMA_VALUE_TYPES = frozenset(
    path.stem for path in (ROOT / "src/commonMain/kotlin/net/multigesture/kanama/types").glob("*.kt")
)
OBJECTCALLS_EXPECT = (
    ROOT / "src/commonMain/kotlin/net/multigesture/kanama/binding/runtime/ObjectCalls.expect.kt"
)
...
# member must be actualized on BOTH backends. Nothing is lost: the shared api tree is compiled per
# platform, so a call to an overload a backend does not have fails that backend's compile, which is
# the same safety net the tree has always had. The helper NAME is still under contract through its
# matched overload.
 ...
 *
 * An `actual object` may declare MORE members than its `expect`: the {desktop_only} desktop-only
 * helpers the tree never calls, the iOS-only overloads, and every private marshalling helper on
 * both sides stay as they are, unmarked.
 *
 * {excluded_note}
 */
...
def desktop_objectcalls_declarations() -> list[tuple[str, str]]:
    ...
    The signature text runs from `fun` to the return type, without the body -- the shape an `expect`
    member needs. Both ObjectCalls files are ktfmt-formatted, so a member starts at exactly two
    spaces (see `check_objectcalls_parity.py`, which relies on the same invariant).
    """
    src = DESKTOP_OBJECTCALLS.read_text(encoding="utf-8")
    out: list[tuple[str, str]] = []
    for match in re.finditer(
        r"^  (?:actual\s+)?fun\s+(?:<[^>]*>\s*)?(\w+)\s*\(", src, re.MULTILINE
    ):
        name = match.group(1)
        open_paren = match.end() - 1
        params = _read_parens(src, open_paren)
        close = open_paren + len(params) + 1
        rest = src[close + 1 :]
        # `): Ret {` or `): Ret =` -- take the return type up to the body.
        body = min(
            (i for i in (rest.find(" {"), rest.find(" =")) if i >= 0),
            default=-1,
        )
        if body < 0:
            raise SystemExit(f"{name}: cannot find the body of the desktop ObjectCalls member")
        ret = rest[:body].strip()
        # 'fun x', 'fun <T> x' or 'actual fun x' -- the `expect` member carries no modifier.
        head = src[match.start() + 2 : open_paren].replace("actual ", "", 1)
        signature = f"{head}({params}){ret}".replace("\n", " ")
        signature = re.sub(r"\s+", " ", signature).replace("( ", "(").replace(" )", ")")
        # ktfmt wraps a long parameter list and leaves a trailing comma; collapse it back to the
        # one-line shape and let ktfmtFormat re-wrap the generated file.
...
def _inject_ios_helper_import(content: str) -> str:
    """Star-import the binding.runtime package into an iOS-ONLY generated file.

    Those files live in `src/iosMain` and their iOS-only sugar can call a hand-written helper that
    is still an `ObjectCalls` EXTENSION, and an extension needs the import. The SHARED tree does not
    get the line: task 104 step 3 parcel C' measured that every symbol a shared file uses
    (RawSegment, NULL_SEGMENT, ObjectCalls, BuiltinCalls) is imported by name, and a common
    `expect object` is not reachable through a star import of a platform package anyway.
    """
...
# A member function declaration: the `actual` modifier (if any) and the name. Whitespace-tolerant on
# purpose: ktfmt may break a long generic parameter list, and the previous, stricter spelling of this
# regex silently missed three wrapped helpers, which made every regen rewrite the generated file raw
# (task 104 parcel A). The modifier is part of the compared unit since parcel C' -- a member that
# loses its `actual` no longer actualizes the `expect object` member, which is drift.
IOS_MEMBER_NAME_RE = re.compile(r"\b(actual\s+)?fun\s+(?:<[^>]*>\s*)?(\w+)\s*\(")


def expect_member_keys(members: list[str]) -> set[tuple[str, tuple[str, ...]]]:
    """(name, parameter names) for each rendered `expect` member -- the key an `actual` must match."""
    keys: set[tuple[str, tuple[str, ...]]] = set()
    for member in members:
        match = re.match(r"\s*fun\s+(?:<[^>]*>\s*)?(\w+)\s*\(", member)
        if match is None:
            raise SystemExit(f"cannot key expect member: {member[:80]}")
        params = _split_top_level(_read_parens(member, match.end() - 1))
        keys.add(
            (
                match.group(1),
                tuple(p.split(":", 1)[0].strip().split()[-1] for p in params if p.strip()),
            )
        )
    return keys


def mark_actual_members(text: str, keys: set[tuple[str, tuple[str, ...]]]) -> tuple[str, int]:
    """Prefix `actual ` on every member of [text] whose (name, parameter names) is in [keys].

    Members of an `actual object` need the modifier one by one (the compiler rejects the object
    otherwise), and an overload the `expect` does not declare must NOT get it -- so the match is on
    the parameter-name tuple, not just the name.
    """
    out: list[str] = []
    marked = 0
    position = 0
    for match in re.finditer(r"^  fun\s+(?:<[^>]*>\s*)?(\w+)\s*\(", text, re.MULTILINE):
        open_paren = match.end() - 1
        params = _split_top_level(_read_parens(text, open_paren))
        key = (
            match.group(1),
            tuple(p.split(":", 1)[0].strip().split()[-1] for p in params if p.strip()),
        )
        if key not in keys:
            continue
        out.append(text[position : match.start() + 2])
        out.append("actual ")
...
def ios_generated_member_names(source: str) -> set[str]:
    """Helper names (with their `actual` modifier) inside the GENERATED MEMBERS region of an iOS
    `ObjectCalls.kt`.
    ...
    """
    return {
        ("actual " if match.group(1) else "") + match.group(2)
        for match in IOS_MEMBER_NAME_RE.finditer(ios_generated_region(source))
    }
...
def regenerate_tree(api_path: Path, only: set[str] | None = None) -> TreeResult:
    ...
    try:
        ...
        with _mode(True, ios_universe):
            ...
            print(
                f"[generate_api_wrapper] iOS ObjectCalls region: {marked} member(s) marked actual"
            )
            files[_rel(IOS_OBJECTCALLS)] = splice_ios_generated_members(
                IOS_OBJECTCALLS.read_text(encoding="utf-8"), region
            )
    finally:
        RENDER_TARGET = previous_target
...
def ios_arg_layout(kind: str, index: int) -> tuple[str, str, list[str], str]:
    ...
    if typed_object_element is not None:
        ...
        # `List<*>`, not `List<GodotObject>`: desktop spells this argument `List<*>`, and since
        # task 104 step 3 parcel C' these helpers are `actual` members of an `expect object`, which
        # the compiler holds to the same parameter TYPES as well as the same names. The iOS
        # descriptor helper takes `List<*>` too and casts each element, exactly as desktop's
        # `BuiltinTypes.initArrayOfObjects` does.
        return (
            "List<*>",
            "PT_TYPED_ARRAY_BLOB",
...
def render_ios_helper(
    ...
    if typed_object_list_return:
        ...
        rendered = (
            # `<T : Any>` mirrors desktop's bound: an `actual` must repeat the expect member's
            # upper bounds (task 104 step 3 parcel C').
            f"  fun <T : Any> {function}({', '.join(params)}): List<T> =\n"
...
def main() -> int:
    ...
    if args.ios_classes:
        ...
        if args.ios_objectcalls:
            ...
            args.ios_objectcalls.write_text(region, encoding="utf-8")
