#!/usr/bin/env python3
"""Generate docs/contributing/gates.md: every gate, what it proves, where it runs (task 99, R18).
...
Usage:
    python3 scripts/generate_gates_index.py --markdown docs/contributing/gates.md
    python3 scripts/generate_gates_index.py --markdown docs/contributing/gates.md --check
"""
...
def render(carried: dict[str, str] | None) -> str:
    ...
    out("# Gates Index")
    out("")
    out("Generated by `python3 scripts/generate_gates_index.py --markdown docs/contributing/gates.md`; `--check` runs as a `local_ci.sh` stage, so this page cannot drift from the scripts and workflows it lists.")
    out("")
