...
class NavigationAgent2D(handle: MemorySegment) : Node(handle) {
    ...
    var debugPathCustomLineWidth: Double
        @JvmName("debugPathCustomLineWidthProperty")
        get() = getDebugPathCustomLineWidth()
        @JvmName("setDebugPathCustomLineWidthProperty")
        set(value) = setDebugPathCustomLineWidth(value)
     ...
     * When `velocity` is used and the processing is completed a `safe_velocity` Vector2 is received
     ...
     * normal pathfinding. To change an actor's pathfinding radius bake `NavigationPolygon` resources
     * with a different `NavigationPolygon.agent_radius` property and use different navigation maps for
     * each actor size.
     *
    ...
    /**
     * If `debug_use_custom` is `true` uses this line width for rendering paths for this agent instead
