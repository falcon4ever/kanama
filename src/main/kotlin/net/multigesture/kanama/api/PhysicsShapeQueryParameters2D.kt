 ...
 */
class PhysicsShapeQueryParameters2D(handle: MemorySegment) : RefCounted(handle) {
    ...
    companion object {
        @JvmStatic
        fun fromHandle(handle: MemorySegment): PhysicsShapeQueryParameters2D? =
            wrap(handle)

        internal fun wrap(handle: MemorySegment): PhysicsShapeQueryParameters2D? =
            if (handle.address() == 0L) null else PhysicsShapeQueryParameters2D(handle)
