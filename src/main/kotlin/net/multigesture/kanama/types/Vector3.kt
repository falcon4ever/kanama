...
data class Vector3(
     ...
     * `(0, 0, 0)` if `v.length() == 0`. See also `is_normalized`. Note: This function may return
    ...
    /**
     * Returns the cross product of this vector and `with`. This returns a vector perpendicular to both
     * this and `with`, which would be the normal vector of the plane defined by the two vectors. As
     * there are two such vectors, in opposite directions, this method returns the vector defined by a
     * right-handed coordinate system. If the two vectors are parallel this returns an empty vector,
    ...
    fun bounce(normal: Vector3): Vector3 =
        callVector3RetVector3("bounce", BOUNCE_HASH, normal)
    ...
    /**
     * Returns the result of rotating this vector around a given axis by `angle` (in radians). The axis
     * must be a normalized vector. See also `@GlobalScope.deg_to_rad`.
     *
     ...
     * counter-clockwise direction and negative in a clockwise direction when viewed from the side
    ...
    private fun callVector3FloatRetVector3(method: String, hash: Long, vector: Vector3, value: Double): Vector3 {
        Arena.ofConfined().use { arena ->
            val base = arena.allocate(GodotReal.SIZE_BYTES * 3, GodotReal.ALIGN_BYTES)
    ...
    private fun callVector3Vector3RetDouble(method: String, hash: Long, first: Vector3, second: Vector3): Double {
        Arena.ofConfined().use { arena ->
            val base = arena.allocate(GodotReal.SIZE_BYTES * 3, GodotReal.ALIGN_BYTES)
    ...
    companion object {
         ...
         * assets like characters, which face towards the camera by convention. Use `Vector3.MODEL_FRONT`
